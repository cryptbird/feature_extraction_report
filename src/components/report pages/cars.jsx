...
const CARS = () => {
  ...
  const { carsFormData } = testData;

  const age = (carsFormData?.Age || "N/A") + " Years";
  const TOT_CARS = testData.psychologistformtestsData["carsscore"] || "N/A";
  const carsInterpretation = TOT_CARS === "" ? "" :
  TOT_CARS < 15 ? "No Risk of Autism" :
  TOT_CARS <= 29.5 ? "Minimal to No Risk of Autism" :
  TOT_CARS <= 36.5 ? "Mild to Moderate Risk of Autism" :
  "Severe Risk of Autism";
  const name=testData.patienthistoryform1data?.NameOfChild
