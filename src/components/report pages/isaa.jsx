...
const ISAA = () => {
  ...
  useEffect(() => {
    ...
    const isaa_social_responsiveness =
      ...
      // console.log("name ",testData.psychologistformtestsData["carsFormData"][`section-0`])
  ...
  return (
    <>
        ...
        <div className="pdf-page bg-white p-8 shadow-md rounded-md w-[210mm] h-[297mm]">
          ...
          >
            ...
            <div style={{ textAlign: "justify" }}>
              ...
              tool designed to evaluate autism spectrum disorder (ASD) in
              children. It assesses social, emotional, communication,
              behavioral, sensory, and cognitive aspects through structured
              observations and parent interviews. The ISAA score helps determine
              the severity of autism, categorizing it as mild, moderate, or
              severe. A higher score indicates more pronounced autism-related
              challenges. It is widely used in clinical, research, and
              diagnostic settings in India.
              <br />
              The system provides outputs as{" "}
              <span
                id="isaaInterpretationmessage"
                style={{ fontSize: "15px", color: "#007bff" }}
              >
                
                {patientData.isaaInterpretation}
              </span>{" "}
              and the Aignosis ISAA test output was:{" "}
              <span
                id="isaascore"
            ...
            <div id="patientData" style={{ fontSize: "larger" }}>
              <div>
                ...
                <span id="isaapatientNamecell"></span>
              </div>
              <div>
                <span className="label">Age:</span>{" "}
                <span id="chronologicalAgecell"></span>
              </div>
            </div>
            <br />
            <br />
      ...
      <div>{patientData.socialResponsiveness}</div>
      <div>{patientData.emotionalResponsiveness}</div>
      <div>{patientData.speechRecognition}</div>
      <div>{patientData.behaviouralPattern}</div>
      <div>{patientData.sensoryAspects}</div>
      <div>{patientData.cognitiveComponent}</div> */}
